...
import { Compass, Briefcase, BarChart3, Shield, Citrus, LogIn, LogOut, Sun, Moon, BookOpen } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center h-14 gap-6">
          ...
          <nav className="hidden md:flex items-center gap-1 flex-1">
            {navItems.map((item) => {
              ...
              const isSecondary = item.path === '/bookbuilding';
              return (
                <Link
                  key={item.path}
                  to={item.path}
                ...
                >
                  <Icon className={cn('h-4 w-4', isSecondary && !active && 'opacity-60')} />
          ...
          <div className="flex items-center gap-2 ml-auto">
            <button onClick={toggle} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors" aria-label="Toggle theme">
              {theme === 'dark' ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
            </button>
            {user ? (
              <div className="flex items-center gap-2">
                <span className="text-[11px] text-muted-foreground hidden sm:inline truncate max-w-[140px]">{user.email}</span>
                <button onClick={() => signOut()} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors">
                  <LogOut className="h-3.5 w-3.5" /> Sign Out
                </button>
              </div>
            ) : (
              <Link to="/auth" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors">
